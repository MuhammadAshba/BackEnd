...
const {getIn}=require('../Controller/foodController')


// get Route
router.route('/get/foods').get(getIn)
...
router.post('/post/food', upload.fields([{ name: 'coverPicture', maxCount: 1 }, { name: 'pictureOfVenue', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        await Food.create(distilled_obj);
